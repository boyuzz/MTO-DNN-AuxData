...
class FgvcAircraft(data.Dataset):
	...
	label_train_filename = "images_variant_trainval.txt"
	label_test_filename = "images_variant_test.txt"
	...
	def __del__(self):
		if self.create_lmdb and self.env:
			self.env.close()
		...
		if self.train:
			...
			with open(label_path, 'r') as f:
				...
				for line in content:
					...
					self.data_label_dict.append((os.path.join(self.root, 'images', fpath), numeric_label_dict[label_name]))
			...
			with open(label_path, 'r') as f:
				...
				for line in content:
					...
					self.data_label_dict.append(
						(os.path.join(self.root, 'images', fpath), numeric_label_dict[label_name]))
