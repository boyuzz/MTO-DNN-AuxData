...
class ImageNet32(data.Dataset):
	...
	"""
	train_base_folder = 'Imagenet32_train'
	test_base_folder = 'Imagenet32_val'
	...
	def __init__(self, opt, train=True,
		...
		if self.train:
			...
			for fentry in self.train_list:
				f = fentry[0]
				file = os.path.join(self.root, self.train_base_folder, f)
				fo = open(file, 'rb')
				if sys.version_info[0] == 2:
			...
			file = os.path.join(self.root, self.test_base_folder, f)
			fo = open(file, 'rb')
			if sys.version_info[0] == 2:
				entry = pickle.load(fo)
			else:
