...
class StanfordDogs(data.Dataset):
	...
	label_train_filename = "train_list.mat"
	label_test_filename = "test_list.mat"
	...
	def __init__(self, opt, train=True, transform=None, target_transform=None,):
		...
		if self.train:
			label_path = os.path.join(self.root, 'lists', self.label_train_filename)
			label_mat = mat4py.loadmat(label_path)
			for i, fpath in enumerate(label_mat['file_list']):
				self.data_label_dict.append((os.path.join(self.root, 'images/Images', fpath[0]), int(label_mat['labels'][i][0]) - 1))
			...
			if self.create_lmdb:
				...
				if not os.path.exists(lmdb_save_path):
					util.create_lmdb(self.data_label_dict, lmdb_save_path, multiplier=7)
			...
			label_path = os.path.join(self.root, 'lists', self.label_test_filename)
			label_mat = mat4py.loadmat(label_path)
			for i, fpath in enumerate(label_mat['file_list']):
				self.data_label_dict.append((os.path.join(self.root, 'images/Images', fpath[0]), int(label_mat['labels'][i][0]) - 1))
